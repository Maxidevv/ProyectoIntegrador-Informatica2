...
// --- DEFINICIONES SIMPLIFICADAS EN ESPANOL ---
#define LONGITUD_MAX_LINEA 1024
#define TAMANO_BLOQUE_INICIAL 10
#define NOMBRE_ARCHIVO "Recursos-reservas.csv"
...
// Funcion simple de delay (reemplazo de Sleep/wait)
void simple_delay(int seconds) {
...
int cargar_datos_csv(DatosMaestros *datos) {
	...
	datos->registros = (Registro*)malloc(TAMANO_BLOQUE_INICIAL * sizeof(Registro));
	datos->cantidad = 0;
	datos->capacidad = TAMANO_BLOQUE_INICIAL;
	if (datos->registros == NULL) {
...
void analizar_tendencia(DatosMaestros *datos) {
	if (datos->cantidad == 0) {
		printf("Error,datos no cargados, el archivo esta vacia\n");
		return;
	}
	mostrar_hora_actual();
	printf("Iniciando analisis de Tendencia Historica.\n");
	...
	if (esta_vacia(&pila_analisis)) {
		printf("No se encontraron datos para el pais y rango de anios.\n");
		return;
	}
	...
	while (!esta_vacia(&pila_analisis)) {
		...
		printf("  Recursos: %.2f (Variacion: %.2f)\n", reciente->recursos, cambio_rec);
		printf("  Reservas: %.2f (Variacion: %.2f)\n", reciente->reservas, cambio_res);
...
void calcular_top_n(DatosMaestros *datos) {
	...
	DatosMaestros datos_auxiliares = {NULL, 0, TAMANO_BLOQUE_INICIAL};
	datos_auxiliares.registros = (Registro*)malloc(TAMANO_BLOQUE_INICIAL * sizeof(Registro));
...
int main() 
{
	...
	{	
		printf("\n");
		printf("_______________________[OPCIONES]__________________________\n");
		printf("\n");
